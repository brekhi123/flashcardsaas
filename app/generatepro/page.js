...
export default function GeneratePro() {
    ...
    const handleSubmit = async () => {
        ...
        fetch('api/generatepro', {
            method: 'POST',
            body: text,
        })
            .then((res) => res.json())
            .then((data) =>  {
                setProFlashcards(data);
    ...
    const saveFlashcards = async () => {
        ...
        if (docSnap.exists()) {
            ...
            if (collections.find((f) => f.name === name)) {
                ...
                batch.set(userDocRef, { proflashcards: collections }, { merge: true });
            }
        } else {
            batch.set(userDocRef, { proflashcards: [{ name }] });
        }
        const colRef = collection(userDocRef, name);
        proflashcards.forEach((flashcard) => {
        ...
        await batch.commit();
        handleClose();
        router.push('/flashcardspro'); // Use the router to navigate
    };
        ...
        <Container maxWidth='md' sx={{
            ...
            <Box sx={{
                ...
                <Paper sx={{
                    ...
                    <TextField
                        ...
                        label='Enter: Topic - Flashcard Count'
                        fullWidth
                        multiline
                        rows={4}
                        variant="outlined"
            ...
            >
                <Alert onClose={() => setSnackbarOpen(false)} severity="success">
                    Your flashcards have been saved!
                </Alert>
            </Snackbar> */}
